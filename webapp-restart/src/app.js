...
const { SESSION_SECRET, isProd, sessionRetentionDays, sessionTouchIntervalMs, sessionTtlMs } = require("./config/env");
const { clientSafeError } = require("./middleware/errors");
const { assertLoginAllowed, LoginRateLimitError } = require("./security/loginRateLimit");
...
app.use("/ui", express.static(path.join(appRoot, "ui")));
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    try {
      await assertLoginAllowed(pool, { username, ipAddress: getClientIp(req) });
    } catch (error) {
      if (error instanceof LoginRateLimitError) {
        await writeLoginAudit(req, { username, success: false, reason: "Rate limited" });
...
app.post("/api/module/:moduleKey/bulk", requireAuth, async (req, res) => {
  ...
  try {
    ...
    try { await client.query("rollback"); } catch (_err) {}
    console.error("MODULE_BULK_CREATE_ERROR", error);
    res.status(500).json({ error: clientSafeError(error) });
  } finally {
    client.release();
  }
