...
function invalidateLeadershipClientCache() {
  if (leadershipState?.cache) leadershipState.cache.clear();
}
...
function seSubmit() {
  ...
  const runner = google.script.run.withSuccessHandler((res) => {
    ...
    if (res.success) { showToast(seEditId ? "Order updated!" : "Order submitted!", "success"); loadedPages.delete("dashboard"); loadedPages.delete("sales-dash"); invalidateLeadershipClientCache(); setTimeout(seReset, 1000); } else showToast(res.error, "error");
...
function ctSubmitEdit(customerId) {
  ...
  updateCustomerRecord(customerId, payload)
    .then(() => {
      ...
      lookupsCache = null;
      invalidateLeadershipClientCache();
...
function payInit() { document.getElementById("pay-date").valueAsDate = new Date(); document.getElementById("pay-comments").value = ""; google.script.run.withSuccessHandler((d) => { payCustomers = d; }).getCustomerList(); }
...
function payChange() { payActive = null; payEditId = ""; document.getElementById("pay-saveBtn").textContent = "CONFIRM PAYMENT"; document.getElementById("pay-search").value = ""; document.getElementById("pay-comments").value = ""; document.getElementById("pay-searchWrap").classList.remove("hidden"); document.getElementById("pay-chip").classList.add("hidden"); document.getElementById("pay-results").classList.add("hidden"); }
function paySubmit() { const amt = document.getElementById("pay-amt").value; if (!payActive || !amt) { showToast("Missing customer or amount", "error"); return; } const btn = document.getElementById("pay-saveBtn"); btn.disabled = true; btn.textContent = payEditId ? "UPDATING..." : "RECORDING..."; const payload = { cid: payActive.cid, customerName: payActive.name, companyName: payActive.company, customerMobile: payActive.phone, amountPaid: amt, paymentMode: document.getElementById("pay-mode").value, paymentDate: document.getElementById("pay-date").value, aid: payActive.aid || "", comments: document.getElementById("pay-comments").value.trim() }; const runner = google.script.run.withSuccessHandler((res) => { btn.disabled = false; btn.textContent = "CONFIRM PAYMENT"; if (res.success) { showToast(payEditId ? "Payment updated" : "Payment saved", "success"); loadedPages.delete("paydash"); loadedPages.delete("cp-dash"); invalidateLeadershipClientCache(); payChange(); document.getElementById("pay-amt").value = ""; document.getElementById("pay-date").valueAsDate = new Date(); } }).withFailureHandler((err) => { btn.disabled = false; btn.textContent = payEditId ? "UPDATE PAYMENT" : "CONFIRM PAYMENT"; showToast(err, "error"); }); return payEditId ? runner.updateCustomerPayment(payEditId, payload) : runner.processPayment(payload); }
...
function pdApply() { const now = new Date(); const todayStr = ymd(now); const text = document.getElementById("pd-search").value.toLowerCase().trim(); const filtered = pdRaw.filter((p) => { let mt = false; if (pdFilter === "today") mt = p.dateString === todayStr; else if (pdFilter === "week") { const sun = new Date(now); sun.setDate(now.getDate() - now.getDay()); mt = p.dateString >= ymd(sun) && p.dateString <= todayStr; } else if (pdFilter === "month") mt = p.dateString.substring(0, 7) === todayStr.substring(0, 7); return mt && (!text || `${p.customer} ${p.company} ${p.mobile} ${p.comments}`.toLowerCase().includes(text)); }); let total = 0; document.getElementById("pd-list").innerHTML = filtered.map((p) => { total += p.amount; return `<div class="pay-card"><div><span class="pay-badge date">${formatDate_(p.dateString)}</span> <span class="pay-badge mode">${p.mode}</span><div style="font-weight:700">${p.customer}</div><div class="recent-meta">${p.company} · ${p.mobile}</div>${p.comments ? `<div class="recent-meta">${escapeHtml(p.comments)}</div>` : ""}</div><div style="display:flex;align-items:center;gap:10px;"><div style="font-family:'JetBrains Mono',monospace;color:var(--green);font-weight:700">₹${p.amount.toLocaleString("en-IN")}</div><div class="icon-row"><button class="icon-btn" onclick="paymentEdit('${p.paymentId}')">ED</button><button class="icon-btn" onclick="paymentDelete('${p.paymentId}')">X</button></div></div></div>`; }).join("") || '<div class="empty"><p>No records found</p></div>'; document.getElementById("pd-total").textContent = "₹" + total.toLocaleString("en-IN"); document.getElementById("pd-count").textContent = filtered.length; }
function paymentEdit(id) { const p = pdRaw.find((row) => row.paymentId === id); if (!p) return; showPage("payment"); payEditId = id; payActive = { cid: p.cid, aid: p.aid || "", name: p.customer, company: p.company, phone: p.mobile }; document.getElementById("pay-searchWrap").classList.add("hidden"); document.getElementById("pay-chip").classList.remove("hidden"); document.getElementById("pay-chipName").textContent = p.customer || "-"; document.getElementById("pay-chipSub").textContent = `${p.company || "-"} · ${p.mobile || "-"}`; document.getElementById("pay-amt").value = p.amount || ""; document.getElementById("pay-date").value = toDateInputValue(p.dateString) || todayYmd(); document.getElementById("pay-mode").value = p.mode || "Cash"; document.getElementById("pay-comments").value = p.comments || ""; document.getElementById("pay-saveBtn").textContent = "UPDATE PAYMENT"; }
function paymentDelete(id) { if (!id || !confirm("Delete this payment?")) return; showLoader("Deleting payment..."); google.script.run.withSuccessHandler(() => { hideLoader(); showToast("Payment deleted", "success"); loadedPages.delete("cp-dash"); invalidateLeadershipClientCache(); pdInit(); }).withFailureHandler((e) => { hideLoader(); showToast(e, "error"); }).deleteCustomerPayment(id); }
